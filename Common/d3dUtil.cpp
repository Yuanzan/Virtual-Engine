...
Math::Matrix4 GetTransposedTransformMatrix(const Math::Vector3& right, const Math::Vector3& up, const Math::Vector3& forward, const Math::Vector3& position)
{
	Math::Matrix4 target;
	target[0] = right;
	target[1] = up;
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 10> d3dUtil::GetStaticSamplers()
{
	...
	const CD3DX12_STATIC_SAMPLER_DESC linearCubemapShadowClamp(
		9,
		D3D12_FILTER_COMPARISON_MIN_MAG_LINEAR_MIP_POINT,
		D3D12_TEXTURE_ADDRESS_MODE_WRAP,  // addressU
		D3D12_TEXTURE_ADDRESS_MODE_WRAP,  // addressV
	...
	const std::string& target)
{
	...
	std::wstring filenameWidth;
	filenameWidth.reserve(filename.size());
	for (uint i = 0; i < filename.size(); ++i)
	{
		filenameWidth.push_back(filename[i]);
	}
	hr = D3DCompileFromFile(filenameWidth.c_str(), defines, D3D_COMPILE_STANDARD_FILE_INCLUDE,
...
ComPtr<ID3DBlob> d3dUtil::ReadCompiledShader(
	const std::string& filename)
{
	ComPtr<ID3DBlob> byteCode;
	std::ifstream ifs(filename, std::ios::binary);
	if (!ifs) return byteCode;
	ifs.seekg(0, std::ios::end);
	size_t length = ifs.tellg();
	ifs.seekg(0, std::ios::beg);
	D3DCreateBlob(length, &byteCode);
	ifs.read((char*)byteCode->GetBufferPointer(), length);
	return byteCode;
}
