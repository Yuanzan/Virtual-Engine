...
float4 LinearTraceRay3DSpace(Texture2D _DepthTexture, int NumSteps, float Thickness, float2 BlueNoise, float3 rayPos, float3 rayDir) {
	...
	for (int i = 0;  i < NumSteps; i++) {
		float endDepth = -_DepthTexture.SampleLevel(pointClampSampler, rayPos.xy, 0).r;
...
float4 HiZ_Trace(int NumSteps, float Thickness, float2 RaytraceSize, float3 rayOrigin, float3 rayDir, Texture2D RT_PyramidDepth)
{
    ...
    for (int i = 0; i < NumSteps; ++i) {
        ...
        float MinZPlane = RT_PyramidDepth.SampleLevel(pointClampSampler, Prev_RayPos.xy, level).r;
...
bool RayCast_Diffuse(uint NumSteps, float Roughness, float CompareFactory, float StepOffset, float3 RayStartScreen, float3 RayStepScreen, Texture2D Texture, out float3 OutHitUVz, out float Level)
{
	...
	for(uint i = 0; i < NumSteps; i += 4)
	{
		...
		float4 SampleDepth;
		SampleDepth.x = Texture.SampleLevel(pointClampSampler, (SampleUV0.xy), Level).r;
		SampleDepth.y = Texture.SampleLevel(pointClampSampler, (SampleUV0.zw), Level).r;
		Level += (8.0 / NumSteps);
		
		SampleDepth.z = Texture.SampleLevel(pointClampSampler, (SampleUV1.xy), Level).r;
		SampleDepth.w = Texture.SampleLevel(pointClampSampler, (SampleUV1.zw), Level).r;
		Level += (8.0 / NumSteps);
        ...
        if( any(Hit) ) {
            ...
            for( uint j = 0; j < 4; j++ ) {
                ...
                float  MidDepth = Texture.SampleLevel( pointClampSampler, MidUVz.xy, Level ).r;
...
bool RayCast_Specular(uint NumSteps, float Roughness, float CompareFactory, float StepOffset, float3 RayStartScreen, float3 RayStepScreen, Texture2D Texture, out float3 OutHitUVz, out float Level)
{
	...
	for( uint i = 0; i < NumSteps; i += 4 )
	{
		...
		float4 SampleDepth;
		SampleDepth.x = Texture.SampleLevel(pointClampSampler, (SampleUV0.xy), 0).r;
		SampleDepth.y = Texture.SampleLevel(pointClampSampler, (SampleUV0.zw), 0).r;
		Level += (8.0 / NumSteps) * Roughness;
		
		SampleDepth.z = Texture.SampleLevel(pointClampSampler, (SampleUV1.xy), 0).r;
		SampleDepth.w = Texture.SampleLevel(pointClampSampler, (SampleUV1.zw), 0).r;
		Level += (8.0 / NumSteps) * Roughness;
        ...
        if(any(Hit))
		{
			...
			bHit = OutHitUVz.z >= 1e-7;//Cull Skybox
			break;
		}
...
bool RayCast_All(uint NumSteps, float Roughness, float CompareFactory, float StepOffset, float3 RayStartScreen, float3 RayStepScreen, Texture2D Texture, out float3 OutHitUVz, out float Level)
{
	...
	for (i = 0; i < NumSteps; i += 4)
	{
		...
		float4 SampleDepth;
		{
			[unroll]
			for (uint j = 0; j < 4; j++) {
				SampleDepth[j] = Texture.SampleLevel(pointClampSampler, SamplesUV[j], SamplesMip[j]).r;
			}
		}
