...
float3 Diffuse_Fabric(float3 DiffuseColor, float3 Roughness4)
{
    return Diffuse_Lambert(DiffuseColor) * lerp(1, 0.5, Roughness4);
}

float Diffuse_Burley_NoPi(float LoH, float NoL, float NoV, float Roughness4)
{
	float F90 = 0.5 + 2 * pow2(LoH) * Roughness4;
	float ViewScatter = F_Schlick(1, F90, NoL);
	float LightScatter = F_Schlick(1, F90, NoV);
	return ViewScatter * LightScatter;
}

float3 Diffuse_Burley(float LoH, float NoL, float NoV, float Roughness4, float3 DiffuseColor)
{
	return Diffuse_Burley_NoPi(LoH, NoL, NoV, Roughness4) * (DiffuseColor * Inv_PI);
}

float Diffuse_RenormalizeBurley_NoPi(float LoH, float NoL, float NoV, float Roughness4)
{
	float EnergyBias = lerp(0.0, 0.5, Roughness4);
	float EnergyFactor = lerp(1.0, 1.0 / 0.662, Roughness4);
	float F90 = EnergyBias + 2.0 * pow2(LoH) * Roughness4;
	float LightScatter = F_Schlick( 1, F90, NoL );
	float ViewScatter = F_Schlick( 1, F90, NoV );
	return LightScatter * ViewScatter * EnergyFactor;
}

float3 Diffuse_RenormalizeBurley(float LoH, float NoL, float NoV, float Roughness4, float3 DiffuseColor)
{
	return Diffuse_RenormalizeBurley_NoPi(LoH, NoL, NoV, Roughness4) * (DiffuseColor * Inv_PI);
}

float Diffuse_OrenNayar_NoPi(float VoH, float NoL, float NoV, float Roughness4)
{
	...
	return (C1 + C2) * (1 + Roughness4 * 0.5);
}

float3 Diffuse_OrenNayar(float VoH, float NoL, float NoV, float Roughness4, float3 DiffuseColor)
{
	return Diffuse_OrenNayar_NoPi(VoH, NoL, NoV, Roughness4) * (DiffuseColor * Inv_PI);
}
...
float D_InverseGGX_CharlieNoPi(float NoH, float Roughness)
{
...
{
    return D_InverseGGX_CharlieNoPi(NoH, Roughness) * Inv_PI;
}
...
{
    return GeometrySchlickGGX(NoV, Roughness) * GeometrySchlickGGX(NoV, Roughness);;
}
...
float Vis_SmithGGX_NoPI(float NoL, float NoV, float Roughness)
{
...
{
	return Vis_SmithGGX_NoPI(NoL, NoV, Roughness) / PI;
}

float Vis_SmithGGXCorrelated_NoPI(float NoL, float NoV, float Roughness)
{
	float a = pow2(Roughness);
...
{
	return Vis_SmithGGXCorrelated_NoPI(NoL, NoV, Roughness) / PI;
}
