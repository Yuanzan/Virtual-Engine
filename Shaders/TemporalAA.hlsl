...
float4 Fog(float eyeDepth, float2 screenuv)
{
    ...
    float3 originRandomValue = MNoise(uvw, _RandomSeed);
	float3 randomValue = (originRandomValue - 0.5) / (_FroxelSize.xyz) * float3(1.77777777777, 1.77777777777, 1.77777777777);
	float4 vol = _VolumeTex.SampleLevel(bilinearClampSampler, uvw + randomValue, 0) * 0.5;
    originRandomValue = frac(originRandomValue + 0.5);
    randomValue =  (originRandomValue - 0.5) / (_FroxelSize.xyz) * float3(1.77777777777, 1.77777777777, 1.77777777777);
    vol += _VolumeTex.SampleLevel(bilinearClampSampler, uvw + randomValue, 0) * 0.5;
...
float2 ReprojectedMotionVectorUV(float2 uv, out float outDepth)
    {
        ...
        uint i;
        outDepth = _DepthTex.SampleLevel(pointClampSampler, uv, 0).x;
        float3 result = float3(0, 0,  outDepth);
        
        [unroll]
        for(i = 0; i < 8; ++i){
            neighborhood = _DepthTex.SampleLevel(pointClampSampler, uv, 0, _OffsetArray[i]).x;
...
float4 frag_volume(v2f i) : SV_TARGET
{
    float2 uv = i.texcoord;
    float depth = _DepthTex.SampleLevel(pointClampSampler, uv, 0);
    float eyeDepth = LinearEyeDepth(depth);
    float4 v = Fog(eyeDepth, uv);
    v.w = 1 - v.w;
    return v;
}
...
float4 frag(v2f i) : SV_TARGET
{
   const float ExposureScale = 10;
        ...
        float2 velocity = _MotionVectorTex.SampleLevel(pointClampSampler, closest, 0).xy;
       ...
       /* TopLeft = Tonemap(TopLeft);
        TopCenter = Tonemap(TopCenter);
        TopRight = Tonemap(TopRight);
        MiddleLeft = Tonemap(MiddleLeft);
        MiddleCenter = Tonemap(MiddleCenter);
        MiddleRight = Tonemap(MiddleRight);
        BottomLeft = Tonemap(BottomLeft);
        BottomCenter = Tonemap(BottomCenter);
        BottomRight = Tonemap(BottomRight);*/
        ...
        float AABBScale = lerp(_TemporalClipBounding.x, _TemporalClipBounding.y, max(VelocityWeight, LastVelocityWeight));
...
//////////////////TemporalResolver
        ...
        depthAdaptiveForce = lerp(depthAdaptiveForce, 0.99, VelocityWeight);
